...
<div class="modal  fade" id="updateNode" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" v-if="node_to_update != null" >
  <div class="modal-dialog " role="document">
    <div class="modal-content">
      <div class="modal-header ">
        <h5 class="modal-title text-center " id="exampleModalLabel" style="width:100%;">Modifier Nœud    </h5>
      ...
      <div class="modal-body">
        <div id="node_update_1">
          ...
          <div class="form-group">
            ...
            <textarea  cols="30" class="form-control" rows="4" v-model="node_to_update.description"></textarea>
        ...
        <div id="node_update_2" style="display:none;" >
          ...
          <div class="form-group">
            ...
            <input  class="form-control" placeholder="Saisir une nouvelle instruction" id="update_new_instruction" v-on:keyup.enter="node_to_update.Conseils_instructions.push($event.target.value);$event.target.value=''" >

            <select class="form-control"  multiple >
              <option  v-for="(instruction,key) in node_to_update.Conseils_instructions" 
                      :key="key" :value="instruction" style="cursor: no-drop;" v-on:dblclick="node_to_update.Conseils_instructions.splice(key,1)"
          ...
          <div class="form-group">
            ...
            <select class="form-control"  v-on:change="($event.target.value != null )? node_to_update.decision.intern.push($event.target.value):null;$event.target.value='null'"   >
              <option value="null">Ajouter Engin</option>
              <option  v-for="(engin,key) in Engins" 
                      :key="key" :value="engin.name"  >
            ...
            <select class="form-control"  multiple >
              <option  v-for="(engin,key) in node_to_update.decision.intern" 
                      :key="key" :value="engin" style="cursor: no-drop;" v-on:dblclick="node_to_update.decision.intern.splice(key,1)"
                      title="double-cliquez pour supprimer cet élément">
                      {{engin}}
           ...
           <div class="form-group">
            ...
            <select class="form-control"  v-on:change="($event.target.value != null )? node_to_update.decision.extern.push($event.target.value):null;$event.target.value = 'null'"   >
              <option value="null">Ajouter Une Etablissement Extern</option>
              <option  v-for="(etablissement,key) in etablissement_extern" 
                      :key="key" :value="etablissement.name"  >
            ...
            <select class="form-control"  multiple >
              <option  v-for="(etablissement,key) in node_to_update.decision.extern" 
                      :key="key" :value="etablissement" style="cursor: no-drop;" v-on:dblclick="node_to_update.decision.extern.splice(key,1)"
                      title="double-cliquez pour supprimer cet élément">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-warning" id="update_back" style="display:none;" v-on:click="modal_next_click(false)">Précédent</button>
        <button type="button" class="btn btn-primary" id="update_next"   v-on:click="modal_next_click(true)">suivant</button>
        <button type="button" class="btn btn-primary" id="update_submit" style="display:none;"   v-on:click="submit_node_update()">Confirmer Les Modifications</button>
...
<div class="modal  fade" id="addNode" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" v-if="node_to_add != null" >
  <div class="modal-dialog " role="document">
    <div class="modal-content">
      <div class="modal-header ">
      ...
      <div class="modal-body">
        ...
        <div id="node_add_2" style="display:none;" >
          ...
          <div class="form-group">
            ...
            <input  class="form-control" placeholder="Saisir une nouvelle instruction" id="update_new_instruction" v-on:keyup.enter="node_to_add.Conseils_instructions.push($event.target.value);$event.target.value=''" >

            <select class="form-control"  multiple >
              <option  v-for="(instruction,key) in node_to_add.Conseils_instructions" 
                      :key="key" :value="instruction" style="cursor: no-drop;" v-on:dblclick="node_to_add.Conseils_instructions.splice(key,1)"
                      title="double-cliquez pour supprimer cet élément">
                      {{instruction}}
          ...
          <div class="form-group">
            ...
            <select class="form-control"  v-on:change="($event.target.value != null )? node_to_add.decision.intern.push($event.target.value):null;$event.target.value='null'"   >
              <option value="null">Ajouter Engin</option>
              <option  v-for="(engin,key) in Engins" 
                      :key="key" :value="engin.name"  >
            ...
            <select class="form-control"  multiple >
              <option  v-for="(engin,key) in node_to_add.decision.intern" 
                      :key="key" :value="engin" style="cursor: no-drop;" v-on:dblclick="node_to_add.decision.intern.splice(key,1)"
                      title="double-cliquez pour supprimer cet élément">
                      {{engin}}
           ...
           <div class="form-group">
            ...
            <select class="form-control"  v-on:change="($event.target.value != null )? node_to_add.decision.extern.push($event.target.value):null;$event.target.value = 'null'"   >
              <option value="null">Ajouter Une Etablissement Extern</option>
              <option  v-for="(etablissement,key) in etablissement_extern" 
                      :key="key" :value="etablissement.name"  >
            ...
            <select class="form-control"  multiple >
              <option  v-for="(etablissement,key) in node_to_add.decision.extern" 
                      :key="key" :value="etablissement" style="cursor: no-drop;" v-on:dblclick="node_to_add.decision.extern.splice(key,1)"
                      title="double-cliquez pour supprimer cet élément">
                      <!--{{etablissement.name }} ( {{etablissement._id}} )-->
                      {{etablissement}}
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-warning" id="add_back" style="display:none;" v-on:click="add_modal_next_click(false)">Précédent</button>
        <button type="button" class="btn btn-primary" id="add_next"   v-on:click="add_modal_next_click(true)">suivant</button>
        <button type="button" class="btn btn-primary" id="add_submit" style="display:none;"   v-on:click="submit_node()">Ajouter</button>
            ...
            <div class="mb-3 card">
              ...
              <ol class="breadcrumb" style="back" id="nodes_path">
                <div class="row">
                  ...
                  <span style="cursor:pointer" class="text-primary" v-on:click="selecteNodeFromPath(null)"> <b>Niveau
                      Principal</b> </span>
                </li>

                
                <li class="breadcrumb-item " v-for="(selected_node) in selected_path" :key="selected_node._id">
                  <span style="cursor:pointer" class="text-primary" v-on:click="selecteNodeFromPath(selected_node._id)">
                    {{selected_node.name}} </span>
                </li>

                      <img style="position:absolute;margin-left:96%;" src="@/assets/images/add.png" width="20"  title="Ajouter Un nœud " v-on:click="node_to_update = selected_node" data-toggle="modal" data-target="#addNode">
                ...
                <div class="p-1 slick-slider-sm mx-auto">
                  <div class="widget-chart widget-chart2 text-left p-0">
                    <div class="widget-chat-wrapper-outer">
                     ...
                     <div class="col-4 col-4" v-on:mouseover="nodeInfoDisplay(node)" v-on:mouseleave="nodeInfoDisplay(null)"  v-for="(node) in current_level" :key="node._id" style="margin-bottom:10px;">
              ...
              <div class="p-0 card-body" >
                ...
                <div v-if="selected_node_display != null">
                  ...
                  <div id="description">
                      ...
                      <img style="margin-left:150px;" src="@/assets/images/edit.png" width="20"  title="Modifier ce nœud sélectionné" v-on:click="node_to_update = selected_node" data-toggle="modal" data-target="#updateNode">
                      <img style="margin-left:5px;" src="@/assets/images/remove.png" width="20"  title="Supprimer ce nœud sélectionné">
                      </ol>
                  ...
                  <div id="Conseils_instructions">
                      ...
                      <span style="cursor:pointer" class="text-success text-capitalize" > 
                        <b> Conseils et instructions :  </b> </span>
                      </ol>
                  
                      <div class="row" >
                        <div class="col-12 pl-4 pr-4 " >
                            <p v-for="(instruction,key) in selected_node_display.Conseils_instructions" :key="key" style="margin-bottom:3px;">
                             - {{instruction}}
                            </p>
                        </div>
                  ...
                  <div id="intern_decision">
                      ...
                      <span style="cursor:pointer" class="text-success text-capitalize" > 
                        <b> Moyen D'intervention Recommandée </b> </span>
                      </ol>
                  ...
                  <div id="extern_decision">
                      ...
                      <span style="cursor:pointer" class="text-success text-capitalize" > 
                        <b> Intervention Externe Recommandée </b> </span>
                      </ol>
                  
                      <div class="row" v-for="(d_extern,key) in selected_node_display.decision.extern" :key="key">
                        <div class="col-8 pl-4">
                            <span>{{d_extern}}</span>
                        </div>
...
export default {
  ...
  data: () => ({
    ...
    Engins:[],
    etablissement_extern:[{name:"Police"},{name:"Gendarmerie"},{name:"Conservation des forêts"},{name:"Hopital"}],
    node_to_add:{
        "decision": {
            "intern": [],
            "extern": []
  ...
  methods: {
    ...
    modal_next_click(direction){
      if(direction) // if true then next is clicked
      {
    ...
    nodeInfoDisplay(node){
      if(node == null)
        ...
        this.selected_node_display = node
    ...
    submit_node_update(){
      axios
            .put("http://localhost:8000/api/tree/"+this.parent_node_id,this.node_to_update)
            .then((res) => {
